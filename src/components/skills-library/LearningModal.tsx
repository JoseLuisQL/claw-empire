    ...
    <div className="skills-learn-modal fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4">
      <div className="skills-learn-modal-card w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/95 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-700/60 px-5 py-4">
          <div>
            <h3 className="text-base font-semibold text-white">
              {t({
                ...
                ja: "スキル学習スクワッド",
                zh: "技能学习小队", es: "Escuadrón de aprendizaje de habilidades"
              })}
            </h3>
          ...
          >
            {learnInProgress
              ? t({ ko: "학습중", en: "Running", ja: "実行中", zh: "进行中", es: "En ejecución" })
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4 max-h-[calc(90vh-72px)]">
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-3 py-2">
            <div className="text-[11px] text-emerald-200">
              {t({ ko: "실행 명령", en: "Install command", ja: "実行コマンド", zh: "执行命令", es: "Comando de instalación" })}
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-slate-400">
              {t({
                ...
                zh: "选择 CLI 代表（可多选）", es: "Selecciona representantes CLI (selección múltiple)"
              })}
            </div>
            <div className="text-[11px] text-slate-500">
              {selectedProviders.length}
              {t({ ko: "명 선택됨", en: " selected", ja: "名を選択", zh: " 已选择", es: " seleccionados" })}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {representatives.map((row) => {
              ...
              const displayName = row.agent
                ...
                : t({ ko: "배치된 인원 없음", en: "No assigned member", ja: "担当メンバーなし", zh: "暂无成员", es: "Sin miembro asignado" });
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    >
                      ...
                      {isHitAnimating && (
                        <span className="unlearn-hit-text">
                          {t({ ko: "깡~", en: "Bonk!", ja: "ゴン!", zh: "咣~", es: "¡Pum!" })}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {row.agent
                          ...
                          : t({ ko: "사용 불가", en: "Unavailable", ja: "利用不可", zh: "不可用", es: "No disponible" })}
                      </div>
                    </div>
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1.5">
                      ...
                      >
                        {isAlreadyLearned
                          ? t({ ko: "학습됨", en: "Learned", ja: "学習済み", zh: "已学习", es: "Aprendido" })
                          : isSelected
                            ? t({ ko: "선택됨", en: "Selected", ja: "選択", zh: "已选", es: "Seleccionado" })
                            : t({ ko: "대기", en: "Idle", ja: "待機", zh: "待命", es: "Inactivo" })}
                      </div>
                      {isAlreadyLearned && (
                        <button
                          type="button"
                        ...
                        >
                          {isUnlearning
                            ? t({ ko: "취소중...", en: "Unlearning...", ja: "取消中...", zh: "取消中...", es: "Desaprendiendo..." })
                            : t({ ko: "학습 취소", en: "Unlearn", ja: "学習取消", zh: "取消学习", es: "Desaprender" })}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-xl border border-slate-700/70 bg-slate-800/55 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="text-slate-300">
                {t({ ko: "작업 상태", en: "Job status", ja: "ジョブ状態", zh: "任务状态", es: "Estado del trabajo" })}:{" "}
                <span
              ...
              <div className="mt-2 rounded-lg border border-slate-700 bg-slate-900/70 p-2 font-mono text-[10px] text-slate-300 max-h-32 overflow-y-auto space-y-1">
                ...
                {learnJob.logTail.length > 0 ? (
                  ...
                  <div className="text-slate-600">
                    {t({ ko: "로그가 아직 없습니다", en: "No logs yet", ja: "ログはまだありません", zh: "暂无日志", es: "Aún no hay registros" })}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {learnSubmitting || learnInProgress
                ? t({ ko: "학습중...", en: "Learning...", ja: "学習中...", zh: "学习中...", es: "Aprendiendo..." })
                : t({ ko: "학습 시작", en: "Start Learning", ja: "学習開始", zh: "开始学习", es: "Iniciar aprendizaje" })}
            </button>
          </div>
        </div>
      </div>
