    ...
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        ...
        <span
          ...
          title={`${t({ ko: "우선순위", en: "Priority", ja: "優先度", zh: "优先级", es: "Prioridad" })}: ${priorityLabel(task.priority, t)}`}
        >
      ...
      <div className="mb-3 flex flex-wrap items-center gap-1.5">
        ...
        {isHiddenTask && (
          <span className="rounded-full bg-cyan-900/60 px-2 py-0.5 text-xs text-cyan-200">
            🙈 {t({ ko: "숨김", en: "Hidden", ja: "非表示", zh: "隐藏", es: "Oculta" })}
          </span>
        )}
        {department && (
        ...
        <div className="flex items-center gap-1.5">
          {assignedAgent ? (
            ...
            <span className="text-xs text-slate-500">
              {t({ ko: "미배정", en: "Unassigned", ja: "未割り当て", zh: "未分配", es: "Sin asignar" })}
            </span>
          )}
        </div>
        ...
        {agentWarning && (
          <p className="mt-1 text-xs font-medium text-red-400 animate-[shake_0.4s_ease-in-out]">
            {t({
              ...
              ja: "担当者を割り当ててください！",
              zh: "请分配负责人！", es: "Por favor, asigna un agente."
            })}
          </p>
        )}
      </div>
                  ...
                  <div key={subtask.id} className="flex items-center gap-1.5 text-xs">
                    ...
                    {subtask.delegated_task_id && subtask.status !== "done" && (
                      <span
                        ...
                        title={t({ ko: "위임됨", en: "Delegated", ja: "委任済み", zh: "已委派", es: "Delegada" })}
                      >
                        🔗
                      </span>
                    )}
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {canRun && (
          <button
            ...
            title={t({ ko: "작업 실행", en: "Run task", ja: "タスク実行", zh: "运行任务", es: "Ejecutar tarea" })}
          ...
          >
            ▶ {t({ ko: "실행", en: "Run", ja: "実行", zh: "运行", es: "Ejecutar" })}
          </button>
        )}
        {canPause && (
          <button
            onClick={() => onPauseTask!(task.id)}
            title={t({ ko: "작업 일시중지", en: "Pause task", ja: "タスク一時停止", zh: "暂停任务", es: "Pausar tarea" })}
          ...
          >
            ⏸ {t({ ko: "일시중지", en: "Pause", ja: "一時停止", zh: "暂停", es: "Pausar" })}
          </button>
        )}
        {canStop && (
          <button
              ...
              if (
                confirm(
                  t({
                    ...
                    zh: `要停止“${task.title}”吗？\n\n警告：停止后将回滚该项目的更改。`,
                    es: `¿Detener "${task.title}"?\n\nAdvertencia: al detener se revertirán los cambios del proyecto.`,
                  }),
                )
              ) {
                onStopTask(task.id);
            ...
            title={t({ ko: "작업 중지", en: "Cancel task", ja: "タスク停止", zh: "取消任务", es: "Cancelar tarea" })}
          ...
          >
            ⏹ {t({ ko: "중지", en: "Cancel", ja: "キャンセル", zh: "取消", es: "Cancelar" })}
          </button>
        )}
        {canResume && (
          <button
            onClick={() => onResumeTask!(task.id)}
            title={t({ ko: "작업 재개", en: "Resume task", ja: "タスク再開", zh: "恢复任务", es: "Reanudar tarea" })}
          ...
          >
            ↩ {t({ ko: "재개", en: "Resume", ja: "再開", zh: "恢复", es: "Reanudar" })}
          </button>
        )}
          ...
          onOpenTerminal && (
            <button
              ...
              title={t({
                ...
                ja: "ターミナル出力を見る",
                zh: "查看终端输出", es: "Ver salida de terminal"
              })}
          ...
          onOpenMeetingMinutes && (
            <button
              ...
              title={t({
                ...
                ja: "会議録を見る",
                zh: "查看会议纪要", es: "Ver acta de reunión"
              })}
        ...
        {task.status === "review" && (
          <button
            ...
            title={t({
              ...
              ja: "変更を見る (Git diff)",
              zh: "查看更改 (Git diff)", es: "Ver cambios (Git diff)"
            })}
          ...
          >
            {t({ ko: "Diff", en: "Diff", ja: "差分", zh: "差异", es: "Diferencias" })}
          </button>
        )}
        {canHideTask && !isHiddenTask && onHideTask && (
          <button
            ...
            title={t({
              ...
              zh: "隐藏已完成/待处理/已取消任务", es: "Ocultar tarea completada/pendiente/cancelada"
            })}
          ...
          >
            🙈 {t({ ko: "숨김", en: "Hide", ja: "非表示", zh: "隐藏", es: "Ocultar" })}
          </button>
        )}
        {canHideTask && !!isHiddenTask && onUnhideTask && (
          <button
            onClick={() => onUnhideTask(task.id)}
            title={t({ ko: "숨긴 작업 복원", en: "Restore hidden task", ja: "非表示タスクを復元", zh: "恢复隐藏任务", es: "Restaurar tarea oculta" })}
          ...
          >
            👁 {t({ ko: "복원", en: "Restore", ja: "復元", zh: "恢复", es: "Restaurar" })}
          </button>
        )}
        {canDelete && (
          <button
              ...
              if (
                confirm(
                  t({
                    ...
                    zh: `要删除“${task.title}”吗？`,
                    es: `¿Eliminar "${task.title}"?`,
                  }),
                )
              )
                onDeleteTask(task.id);
            }}
            title={t({ ko: "작업 삭제", en: "Delete task", ja: "タスク削除", zh: "删除任务", es: "Eliminar tarea" })}
