...
export function taskStatusLabel(status: TaskStatus, t: TFunction) {
  switch (status) {
    case "inbox":
      return t({ ko: "수신함", en: "Inbox", ja: "受信箱", zh: "收件箱", es: "Bandeja de entrada" });
    case "planned":
    ...
    case "done":
      return t({ ko: "완료", en: "Done", ja: "完了", zh: "完成", es: "Completado" });
    case "pending":
...
export function taskTypeLabel(type: TaskType, t: TFunction) {
  switch (type) {
    ...
    case "documentation":
      return t({ ko: "문서화", en: "Documentation", ja: "文書化", zh: "文档", es: "Documentación" });
    default:
      return type;
  }
}
...
export function priorityLabel(priority: number, t: TFunction) {
  if (priority >= 4) return t({ ko: "높음", en: "High", ja: "高", zh: "高", es: "Alta" });
  if (priority >= 2) return t({ ko: "중간", en: "Medium", ja: "中", zh: "中", es: "Media" });
  return t({ ko: "낮음", en: "Low", ja: "低", zh: "低", es: "Baja" });
}
