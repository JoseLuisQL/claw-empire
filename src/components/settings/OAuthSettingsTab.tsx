    ...
    <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">
          {t({ ko: "OAuth 인증 현황", en: "OAuth Status", ja: "OAuth 認証状態", zh: "OAuth 认证状态", es: "Estado de OAuth" })}
        </h3>
      ...
      {oauthResult && (
        ...
        >
          <span>
            {oauthResult.error
              ? `${t({ ko: "OAuth 연결 실패", en: "OAuth connection failed", ja: "OAuth 接続失敗", zh: "OAuth 连接失败", es: "Falló la conexión OAuth" })}: ${oauthResult.error}`
              : `${OAUTH_INFO[oauthResult.provider || ""]?.label || oauthResult.provider} ${t({ ko: "연결 완료!", en: "connected!", ja: "接続完了!", zh: "连接成功!", es: "¡conectado!" })}`}
          </span>
          ...
          <span>
            {oauthStatus.storageReady
              ? t({
                  ...
                  es: "El almacenamiento OAuth está activo (clave de cifrado configurada)",
                })
              : t({
                  ...
                  es: "La variable de entorno OAUTH_ENCRYPTION_SECRET no está configurada",
                })}
          </span>
        </div>
      )}
