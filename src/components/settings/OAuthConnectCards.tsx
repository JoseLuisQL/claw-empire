      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {CONNECTABLE_PROVIDERS.map(({ id, label, Logo, description }) => {
          ...
          const localizedDescription =
            id === "github-copilot"
              ? t({
                  ko: "GitHub OAuth (Copilot 포함)",
                  en: "GitHub OAuth (Copilot included)",
                  ja: "GitHub OAuth（Copilot 含む）",
                  zh: "GitHub OAuth（含 Copilot）",
                  es: "GitHub OAuth (incluye Copilot)",
                })
              : id === "antigravity"
                ? t({
                    ko: "Google OAuth (Antigravity)",
                    en: "Google OAuth (Antigravity)",
                    ja: "Google OAuth（Antigravity）",
                    zh: "Google OAuth（Antigravity）",
                    es: "Google OAuth (Antigravity)",
                  })
                : description;
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] text-slate-400 text-center leading-tight">{localizedDescription}</span>
              ...
              {!storageOk ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-yellow-500/20 text-yellow-500">
                  {t({
                    ...
                    ja: "暗号化キーが必要",
                    zh: "需要加密密钥",
                    es: "Se requiere clave de cifrado",
                  })}
                </span>
              ) : (
                <>
                  {isConnected ? (
                    ...
                    <span className="text-[11px] px-2.5 py-1 rounded-lg bg-amber-500/20 text-amber-300 font-medium">
                      {t({ ko: "감지됨", en: "Detected", ja: "検出済み", zh: "已检测", es: "Detectado" })}
                    </span>
                  ) : null}
                    ...
                    deviceCode && deviceStatus === "polling" ? (
                      ...
                      >
                        {isConnected || isDetectedOnly
                          ? t({ ko: "계정 추가", en: "Add Account", ja: "アカウント追加", zh: "添加账号", es: "Agregar cuenta" })
                          : t({ ko: "연결하기", en: "Connect", ja: "接続", zh: "连接", es: "Conectar" })}
                      </button>
                    )
                  ) : (
                    <button
      ...
      {deviceStatus === "complete" && (
        <div className="space-y-1.5">
          <div className="text-xs text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-2 rounded-lg">
            {t({ ko: "GitHub 연결 완료!", en: "GitHub connected!", ja: "GitHub 接続完了!", zh: "GitHub 已连接!", es: "¡GitHub conectado!" })}
          </div>
          <div className="text-[11px] text-slate-400 bg-slate-800/60 border border-slate-700/50 px-3 py-2 rounded-lg">
            {t({
              ...
              es: "Con una suscripción a Copilot puedes usar modelos de IA. Sin ella, la importación de repositorios de GitHub en Project Manager sigue funcionando.",
            })}
          </div>
        </div>
      )}
