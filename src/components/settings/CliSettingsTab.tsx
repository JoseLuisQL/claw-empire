    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: "var(--th-text-primary)" }}>
          {t({ ko: "CLI 도구 상태", en: "CLI Tool Status", ja: "CLI ツール状態", zh: "CLI 工具状态", es: "Estado de herramientas CLI" })}
        </h3>
                ...
                <div key={provider} className="bg-slate-700/30 rounded-lg p-3 space-y-2">
                  <div className="flex items-center gap-3">
                      ...
                      <div className="text-xs text-slate-500">
                        {status.version ??
                          (status.installed
                            ...
                            : t({ ko: "미설치", en: "Not installed", ja: "未インストール", zh: "未安装", es: "No instalado" }))}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      >
                        {status.installed
                          ? t({ ko: "설치됨", en: "Installed", ja: "インストール済み", zh: "已安装", es: "Instalado" })
                          : t({ ko: "미설치", en: "Not installed", ja: "未インストール", zh: "未安装", es: "No instalado" })}
                      </span>
                      {status.installed && (
                        <span
                        ...
                        >
                          {status.authenticated
                            ? t({ ko: "인증됨", en: "Authenticated", ja: "認証済み", zh: "已认证", es: "Autenticado" })
                            : t({ ko: "미인증", en: "Not Authenticated", ja: "未認証", zh: "未认证", es: "No autenticado" })}
                        </span>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-1.5 pl-0 sm:pl-8">
                      <div className="flex min-w-0 flex-col items-stretch gap-1.5 sm:flex-row sm:items-center sm:gap-2">
                        <span className="w-auto shrink-0 text-xs text-slate-400 sm:w-20">
                          {hasSubModel
                            ? t({ ko: "메인 모델:", en: "Main model:", ja: "メインモデル:", zh: "主模型:", es: "Modelo principal:" })
                        ...
                        {cliModelsLoading ? (
                          ...
                          >
                            <option value="">{t({ ko: "기본값", en: "Default", ja: "デフォルト", zh: "默认", es: "Predeterminado" })}</option>
                            {modelList.map((m) => (
                      ...
                      {provider === "codex" && reasoningLevels && reasoningLevels.length > 0 && (
                        <div className="flex min-w-0 flex-col items-stretch gap-1.5 sm:flex-row sm:items-center sm:gap-2">
                          <span className="w-auto shrink-0 text-xs text-slate-400 sm:w-20">
                            {t({ ko: "추론 레벨:", en: "Reasoning:", ja: "推論レベル:", zh: "推理级别:", es: "Razonamiento:" })}
                          </span>
                          <select
                        ...
                        <>
                          <div className="flex min-w-0 flex-col items-stretch gap-1.5 sm:flex-row sm:items-center sm:gap-2">
                            ...
                            {cliModelsLoading ? (
                              ...
                              >
                                <option value="">
                                  {t({ ko: "기본값", en: "Default", ja: "デフォルト", zh: "默认", es: "Predeterminado" })}
                                </option>
                                {modelList.map((m) => (
                                  <option key={m.slug} value={m.slug}>
                                    {m.displayName || m.slug}
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex min-w-0 flex-col items-stretch gap-1.5 sm:flex-row sm:items-center sm:gap-2">
                                <span className="w-auto shrink-0 text-xs text-slate-400 sm:w-20">
                                  {t({ ko: "알바 추론:", en: "Sub reasoning:", ja: "サブ推論:", zh: "子推理:", es: "Razonamiento subagente:" })}
                                </span>
                                <select
      ...
      <p className="text-xs text-slate-500">
        {t({
          ...
          es: "La herramienta CLI de cada agente se puede cambiar en Office haciendo clic en un agente. Configura los modelos de Copilot/Antigravity en la pestaña OAuth.",
        })}
      </p>
    </section>
  );
