  ...
  return (
    <div className="space-y-2">
      <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
        {t({ ko: "인증 상태", en: "Auth Status", ja: "認証状態", zh: "认证状态", es: "Estado de autenticación" })}
      </div>
          ...
          <div key={provider} className="space-y-2 overflow-hidden rounded-lg bg-slate-700/30 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex min-w-0 flex-wrap items-center gap-x-2.5 gap-y-1">
                ...
                {isFileDetected && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-600/50 text-slate-400">
                    {t({ ko: "CLI 감지", en: "CLI detected", ja: "CLI 検出", zh: "检测到 CLI", es: "CLI detectado" })}
                  </span>
                )}
                {isWebOAuth && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400">
                    {t({ ko: "웹 OAuth", en: "Web OAuth", ja: "Web OAuth", zh: "网页 OAuth", es: "OAuth web" })}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                {!isRunnable ? (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300">
                    {t({
                      ...
                      ja: "検出済み（実行不可）",
                      zh: "已检测（不可执行）", es: "Detectado (no ejecutable)"
                    })}
                  </span>
                ) : !isExpired ? (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400">
                    {info.lastRefreshed
                      ? t({ ko: "자동 갱신됨", en: "Auto-refreshed", ja: "自動更新済", zh: "已自动刷新", es: "Actualizado automáticamente" })
                      : t({ ko: "연결됨", en: "Connected", ja: "接続中", zh: "已连接", es: "Conectado" })}
                  </span>
                ) : info.refreshFailed ? (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400">
                    {t({ ko: "갱신 실패", en: "Refresh failed", ja: "更新失敗", zh: "刷新失败", es: "Error al actualizar" })}
                  </span>
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-400">
                    {t({
                      ...
                      ja: "期限切れ — 再認証が必要",
                      zh: "已过期 — 需重新认证", es: "Expirado — se requiere reautenticación"
                    })}
                  </span>
                ) : (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-400">
                    {t({ ko: "만료됨", en: "Expired", ja: "期限切れ", zh: "已过期", es: "Expirado" })}
                  </span>
                )}
                ...
                {info.hasRefreshToken && isWebOAuth && (
                  ...
                  >
                    {refreshing === provider
                      ? t({ ko: "갱신 중...", en: "Refreshing...", ja: "更新中...", zh: "刷新中...", es: "Actualizando..." })
                      : t({ ko: "갱신", en: "Refresh", ja: "更新", zh: "刷新", es: "Actualizar" })}
                  </button>
                )}
                ...
                {isExpired && !info.hasRefreshToken && isWebOAuth && (
                  ...
                  >
                    {t({ ko: "재연결", en: "Reconnect", ja: "再接続", zh: "重新连接", es: "Reconectar" })}
                  </button>
                )}
                ...
                {isWebOAuth && (
                  ...
                  >
                    {disconnecting === provider
                      ? t({ ko: "해제 중...", en: "Disconnecting...", ja: "切断中...", zh: "断开中...", es: "Desconectando..." })
                      : t({ ko: "연결 해제", en: "Disconnect", ja: "接続解除", zh: "断开连接", es: "Desconectar" })}
                  </button>
                )}
              </div>
            </div>
            ...
            {info.requiresWebOAuth && (
              <div className="text-[11px] text-amber-300 bg-amber-500/10 border border-amber-500/20 rounded px-2.5 py-1.5">
                {t({
                  ...
                  zh: "CLI 检测到的凭据不会直接用于 Claw-Empire 执行。请使用 Web OAuth 重新连接。", es: "Las credenciales detectadas por CLI no se usan directamente para ejecutar Claw-Empire. Reconecta con OAuth web."
                })}
              </div>
            )}
            ...
            {(info.scope || expiresAt || info.created_at > 0) && (
              <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
                {info.scope && (
                  <div className="col-span-2">
                    <span className="text-slate-500">
                      {t({ ko: "스코프", en: "Scope", ja: "スコープ", zh: "范围", es: "Alcance" })}:{" "}
                    </span>
                ...
                {expiresAt && (
                  <div>
                    <span className="text-slate-500">{t({ ko: "만료", en: "Expires", ja: "期限", zh: "到期", es: "Expira" })}: </span>
                ...
                {info.created_at > 0 && (
                  <div>
                    <span className="text-slate-500">{t({ ko: "등록", en: "Created", ja: "登録", zh: "创建", es: "Creado" })}: </span>
              ...
              return (
                <div className="flex min-w-0 flex-col items-stretch gap-1.5 pt-1 sm:flex-row sm:items-center sm:gap-2">
                  <span className="w-auto shrink-0 text-xs text-slate-400">
                    {t({ ko: "모델:", en: "Model:", ja: "モデル:", zh: "模型:", es: "Modelo:" })}
                  </span>
                  {modelsLoading ? (
                    <span className="text-xs text-slate-500 animate-pulse">
                      {t({ ko: "로딩 중...", en: "Loading...", ja: "読み込み中...", zh: "加载中...", es: "Cargando..." })}
                    </span>
                    ...
                    >
                      {!currentModel && (
                        <option value="">
                          {t({ ko: "선택하세요...", en: "Select...", ja: "選択してください...", zh: "请选择...", es: "Seleccionar..." })}
                        </option>
                      )}
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-xs text-slate-500">
                        {t({ ko: "모델 목록 없음", en: "No models", ja: "モデル一覧なし", zh: "无模型列表", es: "Sin modelos" })}
                      </span>
                      {provider === "github-copilot" && (
                        <span className="text-[11px] text-amber-400/80">
                          {t({
                            ...
                            zh: "模型需要 GitHub Copilot 订阅。如果仅需导入仓库，可忽略此项。", es: "Los modelos requieren una suscripción a GitHub Copilot. Puedes ignorarlo si solo necesitas importar repositorios."
                          })}
                        </span>
                      )}
                    </div>
              ...
              <div className="space-y-2 rounded-lg border border-slate-600/40 bg-slate-800/40 p-2.5">
                <div className="flex flex-wrap items-center justify-between gap-1.5">
                  <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                    {t({ ko: "계정 풀", en: "Account Pool", ja: "アカウントプール", zh: "账号池", es: "Pool de cuentas" })}
                  </div>
                  <div className="text-[10px] text-slate-500 text-right">
                    {t({
                      ...
                      zh: "支持多账号同时激活 · 优先级数字越小越先执行", es: "Se admiten varias cuentas activas · prioridad más baja se ejecuta primero"
                    })}
                  </div>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        >
                          {account.active
                            ? t({ ko: "활성", en: "Active", ja: "有効", zh: "活动", es: "Activa" })
                            : t({ ko: "대기", en: "Standby", ja: "待機", zh: "待命", es: "En espera" })}
                        </span>
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded ${
                        ...
                        >
                          {account.executionReady
                            ? t({ ko: "실행 가능", en: "Runnable", ja: "実行可能", zh: "可执行", es: "Ejecutable" })
                            : t({ ko: "실행 불가", en: "Not runnable", ja: "実行不可", zh: "不可执行", es: "No ejecutable" })}
                        </span>
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                        <label className="space-y-1">
                          <span className="block text-[10px] uppercase tracking-wider text-slate-500">
                            {t({ ko: "라벨", en: "Label", ja: "ラベル", zh: "标签", es: "Etiqueta" })}
                          </span>
                          <input
                            value={draft.label}
                        ...
                        <label className="space-y-1">
                          <span className="block text-[10px] uppercase tracking-wider text-slate-500">
                            {t({ ko: "모델 오버라이드", en: "Model Override", ja: "モデル上書き", zh: "模型覆盖", es: "Sobrescritura de modelo" })}
                          </span>
                          <select
                          ...
                          >
                            <option value="">
                              {t({
                                ...
                                ja: "プロバイダ既定値を使用",
                                zh: "使用提供方默认值", es: "Usar valor predeterminado del proveedor"
                              })}
                            </option>
                        ...
                        <label className="space-y-1">
                          <span className="block text-[10px] uppercase tracking-wider text-slate-500">
                            {t({ ko: "우선순위", en: "Priority", ja: "優先度", zh: "优先级", es: "Prioridad" })}
                          </span>
                          <input
                            type="number"
                            min={1}
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        >
                          {account.active
                            ? t({ ko: "풀 해제", en: "Pool Off", ja: "プール解除", zh: "移出池", es: "Sacar del pool" })
                            : t({ ko: "풀 추가", en: "Pool On", ja: "プール追加", zh: "加入池", es: "Agregar al pool" })}
                        </button>
                        ...
                        >
                          {account.status === "active"
                            ? t({ ko: "비활성", en: "Disable", ja: "無効化", zh: "禁用", es: "Desactivar" })
                            : t({ ko: "활성화", en: "Enable", ja: "有効化", zh: "启用", es: "Activar" })}
                        </button>
                        ...
                        >
                          {t({ ko: "삭제", en: "Delete", ja: "削除", zh: "删除", es: "Eliminar" })}
                        </button>
                      </div>
