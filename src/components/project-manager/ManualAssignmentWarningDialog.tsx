      ...
      >
        <div className="border-b border-amber-500/30 px-4 py-3">
          <h3 className="text-sm font-semibold text-amber-200">
            {t({ ko: "수동 배정 확인", en: "Manual Assignment Check", ja: "手動割り当て確認", zh: "手动分配确认", es: "Verificación de asignación manual" })}
          </h3>
        </div>
          ...
          <p className="text-sm text-slate-100">
            {warning.reason === "no_agents"
              ? t({
                  ...
                  zh: "当前未选择员工。若继续保存，运行时可能由组长直接执行。是否继续？", es: "No hay agentes seleccionados. Si guardas ahora, la protección del modo manual puede permitir que los líderes ejecuten tareas directamente. ¿Continuar?"
                })
              : t({
                  ...
                  zh: "当前仅选择了组长。若无下属成员，运行时可能由组长直接执行。是否继续？", es: "Solo hay líderes seleccionados. Sin subordinados, la protección del modo manual puede permitir que los líderes ejecuten tareas directamente. ¿Continuar?"
                })}
          </p>
          <div className="rounded-md border border-slate-700 bg-slate-800/60 px-3 py-2 text-[11px] text-slate-300">
            <p>
              {t({ ko: "선택 요약", en: "Selection Summary", ja: "選択サマリー", zh: "选择摘要", es: "Resumen de selección" })}: {stats.total}
            </p>
            <p>
              {t({ ko: "팀장", en: "Leaders", ja: "リーダー", zh: "组长", es: "Líderes" })}: {stats.leaders} ·{" "}
              {t({ ko: "하위 직원", en: "Subordinates", ja: "サブ担当", zh: "下属成员", es: "Subordinados" })}: {stats.subordinates}
            </p>
          </div>
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-700 px-4 py-3">
          ...
          >
            {t({ ko: "계속 저장", en: "Save Anyway", ja: "そのまま保存", zh: "仍然保存", es: "Guardar de todos modos" })}
          </button>
        </div>
      </div>
    </div>
