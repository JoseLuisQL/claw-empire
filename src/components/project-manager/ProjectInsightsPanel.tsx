...
export default function ProjectInsightsPanel({
  ...
  return (
    <div className="min-w-0 space-y-4">
      <div className="min-w-0 rounded-xl border border-slate-700 bg-slate-800/40 p-4">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-white">
            {t({ ko: "프로젝트 정보", en: "Project Info", ja: "プロジェクト情報", zh: "项目信息", es: "Información del proyecto" })}
          </h4>
        ...
        {loadingDetail ? (
          <p className="mt-2 text-xs text-slate-400">
            {t({ ko: "불러오는 중...", en: "Loading...", ja: "読み込み中...", zh: "加载中...", es: "Cargando..." })}
          </p>
        ) : isCreating ? (
          <p className="mt-2 text-xs text-slate-500">
            {t({
              ...
              ja: "新規プロジェクトを入力中です",
              zh: "正在输入新项目", es: "Creando un proyecto nuevo"
            })}
          </p>
        ) : !selectedProject ? (
          <p className="mt-2 text-xs text-slate-500">
            {t({ ko: "프로젝트를 선택하세요", en: "Select a project", ja: "プロジェクトを選択", zh: "请选择项目", es: "Selecciona un proyecto" })}
          </p>
        ) : (
      ...
      <div className="min-w-0 rounded-xl border border-slate-700 bg-slate-800/40 p-4">
        <h4 className="text-sm font-semibold text-white">
          {t({ ko: "작업 이력", en: "Task History", ja: "作業履歴", zh: "任务历史", es: "Historial de tareas" })}
        </h4>
        {!selectedProject ? (
          <p className="mt-2 text-xs text-slate-500">-</p>
        ) : groupedTaskCards.length === 0 ? (
          <p className="mt-2 text-xs text-slate-500">
            {t({ ko: "연결된 작업이 없습니다", en: "No mapped tasks", ja: "紐づくタスクなし", zh: "没有映射任务", es: "No hay tareas vinculadas" })}
          </p>
        ) : (
            ...
            {groupedTaskCards.map((group) => (
              ...
              >
                ...
                <p className="mt-1 break-all text-[11px] text-slate-500">
                  {t({ ko: "담당", en: "Owner", ja: "担当", zh: "负责人", es: "Responsable" })}:{" "}
                  {group.root.assigned_agent_name_ko || group.root.assigned_agent_name || "-"}
                </p>
                <p className="mt-1 text-[11px] text-blue-300">
                  {t({ ko: "하위 작업", en: "Sub tasks", ja: "サブタスク", zh: "子任务", es: "Subtareas" })}: {group.children.length}
                </p>
                ...
                <p className="mt-2 text-right text-[11px] text-emerald-300">
                  {t({
                    ...
                    ja: "クリックで詳細表示",
                    zh: "点击卡片查看详情", es: "Haz clic en la tarjeta para ver detalles"
                  })}
                </p>
              </button>
            ))}
      ...
      <div className="min-w-0 rounded-xl border border-slate-700 bg-slate-800/40 p-4">
        <h4 className="text-sm font-semibold text-white">
          {t({ ko: "보고서 이력(프로젝트 매핑)", en: "Mapped Reports", ja: "紐づくレポート", zh: "映射报告", es: "Reportes vinculados" })}
        </h4>
        {!selectedProject ? (
          <p className="mt-2 text-xs text-slate-500">-</p>
          ...
          <p className="mt-2 text-xs text-slate-500">
            {t({
              ...
              ja: "紐づくレポートなし",
              zh: "没有映射报告", es: "No hay reportes vinculados"
            })}
          </p>
        ) : (
          <div className="mt-2 max-h-56 overflow-x-hidden overflow-y-auto space-y-2 pr-1">
            {sortedReports.map((row) => (
              ...
              >
                ...
                >
                  {t({ ko: "열람", en: "Open", ja: "表示", zh: "查看", es: "Abrir" })}
                </button>
              </div>
            ))}
          </div>
      ...
      <div className="min-w-0 rounded-xl border border-slate-700 bg-slate-800/40 p-4">
        <h4 className="text-sm font-semibold text-white">
          {t({ ko: "대표 선택사항", en: "Representative Decisions", ja: "代表選択事項", zh: "代表选择事项", es: "Decisiones representativas" })}
        </h4>
        {!selectedProject ? (
          <p className="mt-2 text-xs text-slate-500">-</p>
          ...
          <p className="mt-2 text-xs text-slate-500">
            {t({
              ...
              ja: "代表意思決定の記録はありません",
              zh: "暂无代表决策记录", es: "No hay registros de decisiones representativas"
            })}
          </p>
        ) : (
          <div className="mt-2 max-h-56 overflow-x-hidden overflow-y-auto space-y-2 pr-1">
                ...
                >
                  ...
                  {selectedLabels.length > 0 && (
                    <p className="mt-1 whitespace-pre-wrap break-all text-[11px] text-blue-300">
                      {t({ ko: "선택 내용", en: "Selected Items", ja: "選択内容", zh: "已选内容", es: "Elementos seleccionados" })}:{" "}
                      {selectedLabels.join(" / ")}
                    </p>
                  )}
                  {event.note && event.note.trim().length > 0 && (
                    <p className="mt-1 whitespace-pre-wrap break-all text-[11px] text-emerald-300">
                      {t({ ko: "추가 요청사항", en: "Additional Request", ja: "追加要請事項", zh: "追加请求事项", es: "Solicitud adicional" })}:{" "}
                      {event.note}
                    </p>
                  )}
                </div>
