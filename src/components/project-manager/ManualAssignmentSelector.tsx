        ...
        <div className="mt-2 space-y-3">
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium text-slate-400">
              {t({ ko: "직원 할당 방식", en: "Assignment Mode", ja: "割り当てモード", zh: "分配模式", es: "Modo de asignación" })}
            </span>
            <div className="flex gap-1 rounded-lg border border-slate-700 bg-slate-800 p-0.5">
              ...
              >
                {t({ ko: "자동 할당", en: "Auto", ja: "自動", zh: "自动", es: "Auto" })}
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                {t({ ko: "직접 선택", en: "Manual", ja: "手動", zh: "手动", es: "Manual" })}
              </button>
            </div>
          </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-700 bg-slate-900/50 p-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-400">
                  {t({ ko: "참여 직원 선택", en: "Select Agents", ja: "エージェント選択", zh: "选择员工", es: "Seleccionar agentes" })}
                  <span className="ml-2 font-medium text-blue-400">
                    {selectedAgentIds.size}
                    {t({ ko: "명", en: " selected", ja: "人", zh: "人", es: " seleccionados" })}
                  </span>
                </span>
                {departments.length > 0 && (
                  ...
                  >
                    <option value="all">{t({ ko: "전체 부서", en: "All Depts", ja: "全部署", zh: "所有部门", es: "Todos los departamentos" })}</option>
              ...
              {manualSelectionStats.subordinates === 0 && (
                <p className="text-[11px] text-amber-300">
                  {t({
                    ...
                    zh: "若无下属成员，运行时可能由组长直接执行。", es: "Sin subordinados, los líderes podrían ejecutar tareas directamente."
                  })}
                </p>
              )}
        ...
        <div className="mt-2 rounded-lg border border-violet-500/20 bg-violet-600/10 px-3 py-2">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-violet-400">
              {t({ ko: "직접 선택 모드", en: "Manual Assignment", ja: "手動割り当て", zh: "手动分配", es: "Asignación manual" })}
            </span>
            <span className="text-xs text-slate-400">
              {detail?.assigned_agents?.length ?? 0}
              {t({ ko: "명 지정", en: " agents", ja: "人", zh: "人", es: " agentes" })}
            </span>
          </div>
