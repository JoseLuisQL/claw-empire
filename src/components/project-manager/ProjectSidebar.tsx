    ...
    <aside className="flex w-full flex-col border-r border-slate-700 bg-slate-900/70 md:w-[330px]">
      ...
      <div className="border-b border-slate-700 px-4 py-3">
        ...
        <div className="mt-2 flex gap-2">
          ...
          >
            {t({ ko: "조회", en: "Search", ja: "検索", zh: "查询", es: "Buscar" })}
          </button>
          <button
            type="button"
            onClick={startCreate}
          ...
          >
            {t({ ko: "신규", en: "New", ja: "新規", zh: "新建", es: "Nuevo" })}
          </button>
          <button
            type="button"
            ...
            className="rounded-md bg-slate-700 px-2.5 py-1 text-xs text-slate-200 hover:bg-slate-600"
          >
            {t({ ko: "GitHub 가져오기", en: "GitHub Import", ja: "GitHub インポート", zh: "GitHub 导入", es: "Importar GitHub" })}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loadingList ? (
          ...
          <div className="px-4 py-6 text-xs text-slate-500">
            {t({ ko: "등록된 프로젝트가 없습니다", en: "No projects", ja: "プロジェクトなし", zh: "暂无项目", es: "Sin proyectos" })}
          </div>
        ) : (
      ...
      <div className="flex items-center justify-between border-t border-slate-700 px-4 py-2">
        ...
        >
          {t({ ko: "이전", en: "Prev", ja: "前へ", zh: "上一页", es: "Anterior" })}
        </button>
        ...
        <button
          ...
          className="rounded border border-slate-700 px-2 py-1 text-xs text-slate-300 disabled:opacity-40"
        >
          {t({ ko: "다음", en: "Next", ja: "次へ", zh: "下一页", es: "Siguiente" })}
        </button>
      </div>
    </aside>
  );
