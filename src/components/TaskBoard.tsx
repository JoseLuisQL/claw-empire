    ...
    <div className="taskboard-shell flex h-full flex-col gap-4 bg-slate-950 p-3 sm:p-4">
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-xl font-bold text-white">
          {t({ ko: "업무 보드", en: "Task Board", ja: "タスクボード", zh: "任务看板", es: "Tablero de tareas" })}
        </h1>
        <span className="rounded-full bg-slate-800 px-2.5 py-0.5 text-xs text-slate-400">
          {t({ ko: "총", en: "Total", ja: "合計", zh: "总计", es: "Total" })} {filteredTasks.length}
          {t({ ko: "개", en: "", ja: "件", zh: "项", es: "" })}
          {activeFilterCount > 0 &&
            ` (${t({ ko: "필터", en: "filters", ja: "フィルター", zh: "筛选器", es: "filtros" })} ${activeFilterCount}${t({
              ko: "개 적용",
              en: " applied",
              ja: "件適用",
              zh: "个已应用", es: " aplicados"
            })})`}
        </span>
        <div className="ml-auto flex items-center gap-2">
          {activeFilterCount > 0 && (
            ...
            >
              {t({ ko: "필터 초기화", en: "Reset Filters", ja: "フィルターをリセット", zh: "重置筛选", es: "Restablecer filtros" })}
            </button>
          )}
          <button
            ...
            title={
              showAllTasks
                ? t({
                    ...
                    ja: "進行中表示へ切替（非表示を除外）",
                    zh: "切换到进行中视图（排除隐藏）", es: "Cambiar a vista activa (excluir ocultos)"
                  })
                : t({
                    ko: "모두보기로 전환 (숨김 포함)",
                    en: "Switch to all view (include hidden)",
                    ja: "全体表示へ切替（非表示を含む）",
                    zh: "切换到全部视图（包含隐藏）", es: "Cambiar a vista completa (incluir ocultos)"
                  })
            }
          >
            <span className={showAllTasks ? "text-slate-400" : "text-emerald-200"}>
              {t({ ko: "진행중", en: "Active", ja: "進行中", zh: "进行中", es: "Activo" })}
            </span>
            <span className="mx-1 text-slate-500">/</span>
            <span className={showAllTasks ? "text-cyan-100" : "text-slate-500"}>
              {t({ ko: "모두보기", en: "All", ja: "すべて", zh: "全部", es: "Todo" })}
            </span>
          ...
          <button
            ...
            title={t({
              ...
              zh: "隐藏完成/待处理/已取消任务", es: "Ocultar tareas completadas/pendientes/canceladas"
            })}
          >
          ...
          >
            🗂 {t({ ko: "프로젝트 관리", en: "Project Manager", ja: "プロジェクト管理", zh: "项目管理", es: "Gestor de proyectos" })}
          </button>
          <button
          ...
          >
            + {t({ ko: "새 업무", en: "New Task", ja: "新規タスク", zh: "新建任务", es: "Nueva tarea" })}
          </button>
        </div>
      </div>
              ...
              <div className="flex flex-col gap-2.5 p-2.5 sm:flex-1 sm:overflow-y-auto">
                {columnTasks.length === 0 ? (
                  <div className="flex min-h-24 items-center justify-center py-8 text-xs text-slate-600 sm:flex-1">
                    {t({ ko: "업무 없음", en: "No tasks", ja: "タスクなし", zh: "暂无任务", es: "Sin tareas" })}
                  </div>
                ) : (
