...
export function useNow(localeTag: string, t: TFunction) {
  ...
  const briefing =
    hour < 12
      ? t({ ko: "오전 브리핑", en: "Morning Briefing", ja: "午前ブリーフィング", zh: "上午简报", es: "Briefing matutino" })
      : hour < 18
        ? t({ ko: "오후 운영 점검", en: "Afternoon Ops Check", ja: "午後運用点検", zh: "下午运行检查", es: "Chequeo operativo de la tarde" })
        : t({ ko: "저녁 마감 점검", en: "Evening Wrap-up", ja: "夜間締め点検", zh: "晚间收尾检查", es: "Cierre nocturno" });
...
export function taskStatusLabel(status: string, t: TFunction) {
  switch (status) {
    ...
    case "review":
      return t({ ko: "검토 중", en: "Review", ja: "レビュー", zh: "审核", es: "Revisión" });
    case "done":
      return t({ ko: "완료", en: "Done", ja: "完了", zh: "完成", es: "Completado" });
    case "pending":
      return t({ ko: "보류", en: "Pending", ja: "保留", zh: "待处理", es: "Pendiente" });
    case "cancelled":
      return t({ ko: "취소됨", en: "Cancelled", ja: "キャンセル", zh: "已取消", es: "Cancelado" });
    default:
      return status;
  }
}
