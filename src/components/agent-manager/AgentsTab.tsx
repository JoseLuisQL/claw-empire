  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-3">
        {[
          {
            label: tr("전체 인원", "Total", "Total"),
            value: agents.length,
            icon: <StackedSpriteIcon sprites={randomIconSprites.total} />,
          },
          { label: tr("근무 중", "Working", "En trabajo"), value: workingCount, icon: "💼" },
          { label: tr("부서", "Departments", "Departamentos"), value: departments.length, icon: "🏢" },
        ].map((summary) => (
          <div
            key={summary.label}
      ...
      <div className="flex items-center gap-2 flex-wrap" style={{ borderBottom: "1px solid var(--th-card-border)" }}>
        ...
        >
          {tr("전체", "All", "Todos")} <span className="opacity-60">{agents.length}</span>
        </button>
        {departments.map((department) => {
          ...
          return (
            <button
              ...
              title={tr("더블클릭: 부서 편집", "Double-click: edit dept", "Doble clic: editar depto")}
              className={`flex items-center gap-1 px-3 py-2 text-xs font-medium transition-colors ${
        ...
        <div className="ml-auto pb-1">
          <input
            type="text"
            placeholder={`${tr("검색", "Search", "Buscar")}...`}
            value={search}
      ...
      {sortedAgents.length === 0 ? (
        <div className="text-center py-16" style={{ color: "var(--th-text-muted)" }}>
          ...
          {tr("검색 결과 없음", "No agents found", "No se encontraron agentes")}
        </div>
      ) : (
