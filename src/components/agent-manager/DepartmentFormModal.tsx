  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (api.isApiRequestError(e) && e.code === "department_id_exists") {
        alert(tr("이미 존재하는 부서 ID입니다.", "Department ID already exists.", "El ID del departamento ya existe."));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (api.isApiRequestError(e) && e.code === "department_has_agents") {
        alert(tr("소속 직원이 있어 삭제할 수 없습니다.", "Cannot delete: department has agents.", "No se puede eliminar: el departamento tiene agentes."));
      } else if (api.isApiRequestError(e) && e.code === "department_has_tasks") {
        alert(tr("연결된 업무(Task)가 있어 삭제할 수 없습니다.", "Cannot delete: department has tasks.", "No se puede eliminar: el departamento tiene tareas."));
      } else if (api.isApiRequestError(e) && e.code === "department_protected") {
        alert(tr("기본 시스템 부서는 삭제할 수 없습니다.", "Cannot delete: protected system department.", "No se puede eliminar: departamento del sistema protegido."));
      }
    } finally {
      setSaving(false);
    }
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold flex items-center gap-2" style={{ color: "var(--th-text-heading)" }}>
            ...
            {isEdit
              ? tr("부서 정보 수정", "Edit Department", "Editar departamento")
              : tr("신규 부서 추가", "Add Department", "Agregar departamento")}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
                {tr("영문 이름", "Name", "Nombre")} <span className="text-red-400">*</span>
              </label>
              <input
                type="text"
                value={form.name}
          ...
          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
              {tr("테마 색상", "Theme Color", "Color del tema")}
            </label>
          ...
          {locale.startsWith("ko") && (
            <div>
              <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
                {tr("한글 이름", "Korean Name", "Nombre coreano")}
              </label>
              <input
                type="text"
                value={form.name_ko}
          ...
          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
              {tr("부서 설명", "Description", "Descripción")}
            </label>
            <input
              type="text"
              value={form.description}
              onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder={tr("부서의 역할 간단 설명", "Brief description of the department", "Descripción breve del departamento")}
              className={inputCls}
              style={inputStyle}
            />
          </div>
          ...
          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
              {tr("부서 프롬프트", "Department Prompt", "Prompt del departamento")}
            </label>
            <textarea
              value={form.prompt}
        ...
        <div className="flex items-center gap-2 mt-5 pt-4" style={{ borderTop: "1px solid var(--th-card-border)" }}>
          ...
          >
            {saving
              ? tr("처리 중...", "Saving...", "Guardando...")
              : isEdit
                ? tr("변경사항 저장", "Save Changes", "Guardar cambios")
                : tr("부서 추가", "Add Department", "Agregar departamento")}
          </button>
          {isEdit &&
            (confirmDelete ? (
