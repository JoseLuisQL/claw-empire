      ...
      <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-700 px-4 py-3">
          <h3 className="text-sm font-semibold text-white">
            {tr("프로젝트 분기", "Project Branch", "プロジェクト分岐", "项目分支", "Rama del proyecto")}
          </h3>
          <button
            type="button"
            onClick={onClose}
            ...
            <>
              <p className="text-slate-200">
                {tr(
                  ...
                  "这是已有项目还是新项目？",
                  "¿Este es un proyecto existente o uno nuevo?",
                )}
              </p>
              <div className="flex gap-2">
                ...
                >
                  {tr("기존 프로젝트", "Existing Project", "既存プロジェクト", "已有项目", "Proyecto existente")}
                </button>
                <button
                  type="button"
                  onClick={onChooseNew}
                ...
                >
                  {tr("신규 프로젝트", "New Project", "新規プロジェクト", "新项目", "Proyecto nuevo")}
                </button>
              </div>
            </>
          )}
            ...
            <>
              <p className="text-xs text-slate-400">
                {tr(
                  ...
                  "Mostrando 10 proyectos recientes. Ingresa un número (1-10) o nombre del proyecto.",
                )}
              </p>
              {projectLoading ? (
                <p className="text-xs text-slate-500">
                  {tr("불러오는 중...", "Loading...", "読み込み中...", "加载中...", "Cargando...")}
                </p>
              ) : projectItems.length === 0 ? (
                <p className="text-xs text-slate-500">
                  {tr("프로젝트가 없습니다", "No projects", "プロジェクトなし", "暂无项目", "No hay proyectos")}
                </p>
              ) : (
                <div className="max-h-52 space-y-2 overflow-y-auto pr-1">
                  {projectItems.map((project, idx) => (
                    <div key={project.id} className="rounded-lg border border-slate-700 bg-slate-800/60 p-2">
                      ...
                      >
                        {tr("선택", "Select", "選択", "选择", "Seleccionar")}
                      </button>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {tr("입력값으로 선택", "Select from input", "入力値で選択", "按输入选择", "Seleccionar desde entrada")}
                  </button>
                  <button
                    type="button"
          ...
          {step === "new" && (
            <>
              <input
                ...
                placeholder={tr("프로젝트 이름", "Project name", "プロジェクト名", "项目名称", "Nombre del proyecto")}
              ...
              <input
                ...
                placeholder={tr("프로젝트 경로", "Project path", "プロジェクトパス", "项目路径", "Ruta del proyecto")}
                className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-white outline-none focus:border-blue-500"
              />
              <textarea
                rows={3}
                ...
                placeholder={tr("핵심 목표", "Core goal", "コア目標", "核心目标", "Objetivo principal")}
              ...
              {isDirectivePending && (
                <p className="text-[11px] text-slate-400">
                  {tr(
                    ...
                    "$ 指令内容会自动作为新项目核心目标。",
                    "El texto de la directiva $ se usa automáticamente como objetivo principal del nuevo proyecto.",
                  )}
                </p>
              )}
              <div className="flex gap-2">
                ...
                >
                  {projectSaving
                    ? tr("등록 중...", "Creating...", "作成中...", "创建中...", "Creando...")
                    : tr("등록 후 선택", "Create & Select", "作成して選択", "创建并选择", "Crear y seleccionar")}
                </button>
                <button
                  type="button"
                  onClick={onBackToChoose}
          ...
          {step === "confirm" && selectedProject && (
            <>
              ...
              <div className="rounded-lg border border-blue-700/40 bg-blue-900/20 p-3 text-[11px] text-blue-100">
                <p className="font-medium">{tr("라운드 목표", "Round Goal", "ラウンド目標", "回合目标", "Objetivo de la ronda")}</p>
                <p className="mt-1 leading-relaxed">
                  {tr(
                    ...
                    `Ejecuta esta ronda con el objetivo principal del proyecto (${selectedProject.core_goal}) y la solicitud actual (${pendingContent}).`,
                  )}
                </p>
              </div>
              <div className="flex gap-2">
                ...
                >
                  {tr("선택 후 전송", "Select & Send", "選択して送信", "选择并发送", "Seleccionar y enviar")}
                </button>
                <button
                  type="button"
                  onClick={onBackToChoose}
                  className="rounded border border-slate-700 px-3 py-2 text-xs text-slate-300"
                >
                  {tr("다시 선택", "Re-select", "再選択", "重新选择", "Volver a seleccionar")}
                </button>
              </div>
            </>
          )}
