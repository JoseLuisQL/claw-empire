      ...
      <div className="space-y-3">
        <div className="bg-slate-700/30 rounded-lg p-3">
          <div className="text-xs text-slate-500 mb-1">
            {t({ ko: "성격", en: "Personality", ja: "性格", zh: "性格", es: "Personalidad" })}
          </div>
          <div className="text-sm text-slate-300">
            {agent.personality ?? t({ ko: "설정 없음", en: "Not set", ja: "未設定", zh: "未设置", es: "Sin configurar" })}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-700/30 rounded-lg p-3 text-center">
            ...
            <div className="text-[10px] text-slate-500">
              {t({ ko: "완료 업무", en: "Completed", ja: "完了タスク", zh: "已完成任务", es: "Completadas" })}
            </div>
          </div>
          <div className="bg-slate-700/30 rounded-lg p-3 text-center">
            <div className="text-lg font-bold text-white">{xpLevel}</div>
            <div className="text-[10px] text-slate-500">{t({ ko: "레벨", en: "Level", ja: "レベル", zh: "等级", es: "Nivel" })}</div>
          </div>
          <div className="bg-slate-700/30 rounded-lg p-3 text-center">
            ...
            <div className="text-[10px] text-slate-500">
              {t({ ko: "알바생", en: "Sub-agents", ja: "サブエージェント", zh: "子代理", es: "Subagentes" })}
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          >
            💬 {t({ ko: "대화하기", en: "Chat", ja: "チャット", zh: "对话", es: "Chat" })}
          </button>
          <button
          ...
          >
            📋 {t({ ko: "업무 배정", en: "Assign Task", ja: "タスク割り当て", zh: "分配任务", es: "Asignar tarea" })}
          </button>
        </div>
        {agent.status === "working" && agent.current_task_id && onOpenTerminal && (
          ...
          >
            &#128421; {t({ ko: "터미널 보기", en: "View Terminal", ja: "ターミナル表示", zh: "查看终端", es: "Ver terminal" })}
          </button>
        )}
      </div>
    );
      ...
      {agentSubAgents.length === 0 ? (
        <div className="text-center py-8 text-slate-500 text-sm">
          ...
          {t({
            ...
            ja: "現在サブエージェントはいません",
            zh: "当前没有子代理",
            es: "No hay subagentes actualmente",
          })}
          <div className="text-xs mt-1 text-slate-600">
            {t({
              ...
              zh: "并行处理时会自动生成子代理。",
              es: "Los subagentes se crean automáticamente durante el trabajo en paralelo.",
            })}
          </div>
        </div>
      ) : (
          ...
          >
            <div className="w-8 h-8 rounded-full bg-amber-500/20 overflow-hidden flex items-center justify-center">
              <img
                ...
                alt={t({ ko: "알바생", en: "Sub-agent", ja: "サブエージェント", zh: "子代理", es: "Subagente" })}
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm text-white truncate flex items-center gap-1.5">
                <span className="text-[10px] px-1 py-0.5 rounded bg-amber-500/20 text-amber-400">
                  {t({ ko: "알바", en: "Sub", ja: "サブ", zh: "子任务", es: "Sub" })}
                </span>
                {subAgent.task}
              </div>
              <div className="text-xs text-slate-500 mt-0.5">
                {subAgent.status === "working"
                  ? `🔨 ${t({ ko: "작업중...", en: "Working...", ja: "作業中...", zh: "工作中...", es: "Trabajando..." })}`
                  : `✅ ${t({ ko: "완료", en: "Done", ja: "完了", zh: "完成", es: "Hecho" })}`}
              </div>
            </div>
            {subAgent.status === "working" && (
